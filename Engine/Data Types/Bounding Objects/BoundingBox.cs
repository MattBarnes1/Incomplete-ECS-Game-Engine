...
namespace Engine.DataTypes
{
    ...
    public struct BoundingBox : IEquatable<BoundingBox>
    {
        ...
        public BoundingBox(Vector3 min, Vector3 max)
        {
            this.Min = min;
            this.Max = max;
            Center = ((this.Max - this.Min) * 0.5f) + this.Min;

        }
        public BoundingBox(BoundingBox Extents, Vector3 aCenter)
        {
            this.Center = aCenter;
            Vector3 Offset = aCenter - Extents.Center;
            this.Min = Extents.Min + Offset;
            this.Max = Extents.Max + Offset;
        }

        public Vector3 Center { get; private set; }

        public BoundingBox(ModelComponent myModel, TransformComponent myComponent) : this(myModel.MeshWidthHeight, myComponent.WorldPosition)
        {
        ...
        public static void CreateFromSphere(ref BoundingSphere sphere, out BoundingBox result)
        {
            ...
            result = new BoundingBox(sphere.Center - corner, sphere.Center + corner);
        }

        public static BoundingBox CreateMerged(ref BoundingBox original, ref BoundingBox additional)
        {
            BoundingBox result;
        ...
        public static void CreateMerged(ref BoundingBox original, ref BoundingBox additional, out BoundingBox result)
        {
            result = new BoundingBox(
             new Vector3(Math.Min(original.Min.X, additional.Min.X), Math.Min(original.Min.Y, additional.Min.Y), Math.Min(original.Min.Z, additional.Min.Z)),
             new Vector3(Math.Max(original.Max.X, additional.Max.X), Math.Max(original.Max.Y, additional.Max.Y), Math.Max(original.Max.Z, additional.Max.Z)));
        }
